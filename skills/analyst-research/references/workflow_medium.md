# analyst-research · medium 模式工作流（v0.1）

> **v0.6.0 迁移说明**：本文件由原 `medium-research` skill 整体迁入 analyst-research 作为 medium 模式。文档内提及「medium-research skill」「~/.claude/skills/medium-research/...」等表述均指当前 analyst-research 的 medium 模式与其 references/scripts 文件夹。后续 v0.7 会做一次文字纯化。

适用对象：8-15 页主题分析，3-8 张图，半天预算（3-5 h），单 LLM，1 硬停（step 7 用户 sign off 强制），可选第二硬停（step 2 广搜后，onboarding 时让用户决定）。
...
向用户简短确认：「我已读完 `~/.claude/skills/medium-research/references/workflow.md` 与 `report_style_spec.md`，准备启动 medium-research 8 步流程。」
...
- 篇幅：8-15 页（6-7 页强警告 + 建议转 light；16-20 页强警告 + 建议精简；20+ 页要求用户重新评估是 medium 还是 heavy）
- 图表：3-8 张，**图表前置**（先做图再写正文，沿用 heavy step 7 纪律）
- LLM：单 LLM only（Claude solo 全程），不调多 LLM critique
- 摘要形态：default 用「三段式摘要」（结论 + 关键数字 + so-what）。用户要 BLUF 单段就明说
- 时间预期：3-5 小时单 / 双 session 跑完
...
cp ~/.claude/skills/medium-research/references/_quarto-medium.yml _quarto.yml && \
...
SKILL_SCRIPTS = Path.home() / ".claude" / "skills" / "medium-research" / "scripts"
...
| 4 | 数据 + 图表 3-8 张（一图一脚本、双双落盘、视觉自检 4 项） | 软停 | 60-90 min | **完整继承 heavy 7 纪律**，张数压缩 |
| 5 | draft 一气连写 8-15 页 | 软停 | 45-60 min | heavy 8 default 路径 |
...
**图表清单生成**：Claude 基于资料库给图表清单（每节建议哪些图能支撑论点），目标 **3-8 张**，**已按 step 4「加图判据 6 条」预筛**。
...
1. Claude 按节写初稿（v1.0），**逐节自检 §五 写作纪律**（句号续接 / 列举冒号 / 嵌入 takeaway / ppts 改个百分点 / 书面化连接词 / 序号起头 / 不用技术符号代连词）
...
- PDF 翻一下肉眼看：无 ctex / xelatex 错、无 figure caption 残留、页数在 8-15 范围内
...
- 20+ 页：告诉用户「内容量级已接近 heavy，请重新评估是继续 medium 路径精简到 15 页内，还是转 heavy-research 重新规划」，由用户裁定，AI 不强行硬截
...
medium 与 heavy 共享同一份视觉规范（实文件均在 `~/.claude/skills/medium-research/references/report_style_spec.md`）：
...
1. **不出 `_clean.jpg`**：medium 砍 publication HTML 与公众号派生，`save_fig` 只生成 PDF + JPG 两格式，跳过 `_clean.jpg` 栅格
...
**default：三段式摘要**（与 heavy 对齐，适合 8-15 页主题分析）：
...
**证据要求（反「自检走过场」纪律）**：声明 self-check 全过时**必须粘最后一次实际 grep 输出的数字**，不允许只口头声明「全过」。
...
| 页数 | 渲染 PDF 后翻一下 | 8-15 OK，6-7 强警告转 light，16-20 强警告精简，20+ 用户裁定转 heavy |
...
|---|---|---|---|
| 步骤数 | 6 | **8** | 11 |
| 时间预算 | 60-80 min | **3-5 h** | 多日 |
| 篇幅 | 5 页 / 2500-3000 字 | **8-15 页** | 1.5 万字+ / 35+ 页 |
| 图表 | 0 | **3-8 张** | 通常 15+ 张，常达 30+ |
...
>
> **依赖 skill**：medium-research v0.1（中等深度研究工作流 skill）
...
文风、标点、引用见 `~/.claude/skills/medium-research/references/workflow.md §五`；文档版式见 `references/report_style_spec.md §一`；图表制作见 spec §二 / §三；YAML / 字体 / 调色板默认值见 spec §五。**全部继承不复述**。
...
<若本项目偏离 medium-research workflow / spec 的某条规则，记在这里>
```
...
- **v0.1**（2026-05-18）：初版。从 heavy v1.x 精简 + light v1.0 精简 + 用户 brainstorm 决策（8 步骨架 A 方案、step 2 可选硬停、symlink 改 cp、PDF + Word 派生、footnote 引用、7 个编号子目录）综合而成。
