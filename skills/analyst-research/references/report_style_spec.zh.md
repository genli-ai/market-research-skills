...
> 英文主文件为 `report_style_spec.md`（中文镜像）。
...
>
> 本文档同时作为 `analyst-research` skill 的 `references/report_style_spec.md`。在 skill 中三件套分层位于 `references/`（本文档、workflow.md）与 `scripts/`（chart_template.py）；在用户研报项目里三件套扁平位于 `analyst-research/`。下方所有引用其他两个三件套文件时按文件名直引，不带路径，两种结构都能解析。
...
| 文件 | 位置 | 角色 |
|---|---|---|
| `SKILL.md` | `analyst-research/` | skill 入口（frontmatter + 加载顺序） |
| `workflow.md` | `analyst-research/references/` | 过程纪律：十一步骨架、多 LLM 分工、复盘格式 |
| `report_style_spec.md` | `analyst-research/references/` | 视觉规范（本文档）+ chart_template 接口契约 |
| `chart_template.py` | `analyst-research/scripts/` | 绘图实现 single source of truth：`PALETTE`、`setup_style`、`save_fig`、`legend_above` |
| `publication-style-template.html` | `analyst-research/scripts/` | publication HTML 模板（10c 可选派生用） |
| `author.jpg` | `analyst-research/scripts/` | 作者头像 placeholder（10c 可选派生用） |
| `_path.py` | `5_scripts/` | 4 行 sys.path 注入到 `../analyst-research/scripts/`。新项目脚手架阶段必须先建（内容见 §6.4 末尾） |
...
每个项目的 `analyst-research/scripts/chart_template.py` 是绘图样式 single source of truth。所有 `5_scripts/make_fig_*.py` 顶部 `import _path; from chart_template import setup_style, save_fig, PALETTE`（`_path.py` 见 §6.4）。保证字体、调色板、边距、网格、spine、字号、DPI、输出格式一致。**任何脚本不允许 override 这些样式**，除非用户明确批准。
...
| `clean` | bool | 是否生成 `_clean.jpg`（default True，heavy publication HTML 需要）。medium / light 不出 publication，传 `clean=False` 跳过 |
...
```python
"""把 analyst-research/scripts/ 加进 sys.path，让 5_scripts/ 的脚本能 import chart_template。"""
import sys
